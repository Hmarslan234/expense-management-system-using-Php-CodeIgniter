...
class Home extends CI_Controller {
    ...
    public function manage_category() {
       if ($this->session->userdata('isUserLoggedIn')) {
            $data['user'] = $this->user->getRows(array('id' => $this->session->userdata('userId')));
            $this->load->model("Home_model");
            $data["fetch_item"] = $this->Home_model->fetch_item();
    ...
    public function manage_expense() {
        if ($this->session->userdata('isUserLoggedIn')) {
            ...
            $data["fetch_cus"] = $this->Home_model->fetch_cus();
             $data["fetch_item"] = $this->Home_model->fetch_item();
            $this->load->view('header', $data);
            $this->load->view('manage_expense', $data);
            $this->load->view('footer');
        } else {
            redirect('Users');
        }
    ...
    public function receive_amount() {
        if ($this->session->userdata('isUserLoggedIn')) {
            ...
            $data["fetch_cus"] = $this->Home_model->fetch_cus();
                $data["fetch_item"] = $this->Home_model->fetch_item();
            $this->load->view('header', $data);
            $this->load->view('receive_amount', $data);
            $this->load->view('footer');
        } else {
            redirect('Users');
        }
    ...
    if ($this->session->userdata('isUserLoggedIn')) {
            ...
            if (isset($_POST['cat'])) {
                $userData = array(
                    'items' => strip_tags($this->input->post('items')),
